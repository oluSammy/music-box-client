  ...
  React.useEffect(() => {
    if (filterTxt === '') {
      ...
      tracks.forEach((track: any) => {
        if (track.title.toLowerCase().startsWith(filterTxt.toLowerCase()))
        ...
        songs.map((track: any, idx: number) =>
          <div className={clsx(classes.tableHeading, classes.showOnHover)} key={track._id}>
            ...
            <h5 className={clsx(classes.contentTxt, classes.nameMobile)}>
              ...
              <span className={classes.spanMobile} >{track.artist} / {secondsToHms(+track.duration)}</span>
            </h5>
            ...
            <h5 className={clsx(classes.contentTxt, classes.contentOpacity, classes.hideOnMobile)}>{secondsToHms(+track.duration)}</h5>
