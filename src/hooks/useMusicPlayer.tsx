...
const useMusicPlayer = () => {
    const { currentSong, trackIndex, setTrackIndex, playing, setPlaying, currentSongArray } = useContext(MusicPlayerContext);
    ...
    const playPrev = () => {
        const newIndex = ((trackIndex + -1) % currentSongArray.length + currentSongArray.length) % currentSongArray.length;
        playTrack(newIndex)
    }
    return {
        toggleMusicPlay,
